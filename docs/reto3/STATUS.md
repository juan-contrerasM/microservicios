...
| 3 | Circuit Breaker Resilience4j en `empleados-service` (llamada a departamentos). Tres estados observables. Fallback `PENDIENTE_VALIDACION` + reconciliación mínima | ⬜ | Criterios 3 y 4. Timeout de llamada → 5 s. |
...
**Qué toca ahora: Etapa 3**, solo en `services/empleados-service/` (Resilience4j en la llamada a
departamentos). No poner el Circuit Breaker en el Gateway ni en Go. Contrato y parámetros:
[`PLAN-RETO3.md`](PLAN-RETO3.md) §Etapa 3. Las carpetas 5 y 6 de
[`Reto3.postman_collection.json`](Reto3.postman_collection.json) son de esa etapa: no correrlas
hasta que exista el fallback `PENDIENTE_VALIDACION`.
...
| Librería del Circuit Breaker | Propuesta | Resilience4j en `empleados-service` (quien llama) |
| Parámetros del circuito | Propuesta | 3 fallos lógicos, 30 s en OPEN, 1 llamada en HALF_OPEN, timeout HTTP 5 s |
| Fallback | Propuesta | Persistir con `PENDIENTE_VALIDACION` (disponibilidad). Reconciliación: endpoint/barrido que reconsulta departamentos; nunca departamento por defecto |
